 ...
 *  新增长按手势
 */
@property(nonatomic,strong)UILongPressGestureRecognizer *long_viewGesture;
...
-(void)awakeFromNib{
    ...
    //新增手势
    [self addGestureRecognizer:self.long_viewGesture];
}
...
-(void)dataPrepare{
    ...
    if(isNetWorkShow){//网络请求
        ...
        if (_photoModel.isWhiteBGColor) {
            
            image=[UIImage whiteBGphImageWithSize:[UIScreen mainScreen].bounds.size zoom:.3f];
        }else{
            image= [UIImage blackBGphImageWithSize:[UIScreen mainScreen].bounds.size zoom:.3f];
        }
 ...
 *  长按
 */
-(void)tap_long_imageViewTap:(UILongPressGestureRecognizer *)longtap
{
...
-(void)needCloseView
{
    if (_ItemViewWillCloceBlock) {
        _ItemViewWillCloceBlock();
    }
}

-(void)tap_view{
    
    if (_ItemViewSingleTapBlock) {
